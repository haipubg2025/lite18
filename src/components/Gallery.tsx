...
export default function Gallery({ theme }: GalleryProps) {
  ...
  return (
    <div
      {...getRootProps()}
      className="h-full w-full flex flex-col relative overflow-hidden bg-black outline-none"
    >
      {/* Nút Upload */}
      <div className="absolute top-6 left-6 z-30 flex gap-2">
        <button
          onClick={() => fileInputRef.current?.click()}
          className="p-3 rounded-full bg-white/5 hover:bg-white/20 backdrop-blur-md border border-white/5 text-white transition-all cursor-pointer group"
          title="Tải Ảnh Lên"
        >
          <Upload className="w-5 h-5 opacity-30 group-hover:opacity-100" />
        </button>
        <input
          type="file"
          ref={fileInputRef}
        ...
        >
          {isBlurred ? (
            ...
            <EyeOff className="w-5 h-5 opacity-50 group-hover:opacity-100" />
          )}
        </button>
        {images.length > devGalleryImages.length && (
          <button
            ...
            className="p-3 rounded-full bg-red-500/5 hover:bg-red-500/20 backdrop-blur-md border border-red-500/10 text-red-500 transition-all cursor-pointer group"
            title="Xóa ảnh đã tải"
          >
            <X className="w-5 h-5 opacity-30 group-hover:opacity-100" />
          </button>
        )}
      </div>

      {isDragActive && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm border-4 border-dashed border-white/50 m-8 rounded-3xl">
          ...
          >
            {currentPair.map((url, idx) => (
              <motion.div
                ...
                className="flex-1 h-full min-h-0 min-w-0 relative overflow-hidden shadow-2xl bg-black rounded-xl"
              >
                ...
                >
                  <img
                    src={url}
                    className={`w-full h-full max-h-full max-w-full object-contain relative z-10 transition-all duration-700 group-hover/img:scale-105 ${
                  ...
                  <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover/img:opacity-100 transition-opacity bg-black/20">
                    <Maximize2 className="w-12 h-12 text-white drop-shadow-xl" />
                  </div>
                </div>
                <div className="absolute inset-0 bg-linear-to-t from-black/40 via-transparent to-black/20 pointer-events-none z-20 rounded-xl" />
              </motion.div>
            ))}
            ...
            {currentPair.length < perPage && images.length > 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex-1 h-full relative rounded-3xl overflow-hidden border border-white/5 bg-white/2"
              />
            )}
          </div>
        </AnimatePresence>
      ...
      {/* Thanh tiến trình */}
      <div
        className="absolute bottom-0 left-0 h-0.5 bg-white/20 transition-all duration-100 ease-linear z-40"
        style={{
          width: "100%",
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex items-center gap-6 opacity-0 hover:opacity-100 transition-opacity duration-1000">
        <button
          ...
          className="p-2 text-white/30 hover:text-white transition-all hover:scale-125"
        >
        ...
        <div className="flex gap-2">
          {Array.from({ length: Math.ceil(images.length / perPage) }).map(
            (_, i) => (
              <div
                key={i}
                className={`w-1.5 h-1.5 rounded-full transition-all duration-700 ${Math.floor(currentIndex / perPage) === i ? "bg-white w-6" : "bg-white/10"}`}
              />
            ),
          )}
        </div>
